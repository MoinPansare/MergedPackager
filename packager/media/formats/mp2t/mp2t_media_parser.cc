...
// ECL: Create this so that only the mp2t media format supports SCTE-35 signals
void Mp2tMediaParser::SetSignalCallback(
    const NewSignalCB& new_signal_cb
    ) {

  DCHECK(!is_initialized_);
  DCHECK(!new_signal_cb.is_null());
  
  new_signal_cb_ = new_signal_cb;
}
...
void Mp2tMediaParser::RegisterPes(int pmt_pid,
  ...
  switch (static_cast<TsStreamType>(stream_type)) {
    ...
    case TsStreamType::kScte35:
      es_parser.reset(new EsParserScte35(
	        pes_pid, 
          base::Bind(&Mp2tMediaParser::OnNewSpliceInfoSection, base::Unretained(this))));
      contains_pes = false;
      break;
    default: {
  ...
  if (contains_pes) {
    ...
    DVLOG(1) << "Create a new ES Data state";
    std::unique_ptr<TsSection> data_section_parser(
        new TsSectionData(std::move(es_parser)));
    std::unique_ptr<PidState> data_pid_state(
        new PidState(pes_pid, PidState::kPidData, std::move(data_section_parser)));
    data_pid_state->Enable();
    pids_.insert(std::pair<int, std::unique_ptr<PidState>>(
        pes_pid, std::move(data_pid_state))); 
...
// ECL - This callback is invoked by the SCTE-35 parser
void Mp2tMediaParser::OnNewSpliceInfoSection(
    uint32_t pid,
    const std::shared_ptr<splice_info_section_t>& splice_info) {


  LOG(INFO) << __FUNCTION__;

  // Add the sample to the appropriate PID sample queue.
  PidMap::iterator pid_state = pids_.find(pid);
  if (pid_state == pids_.end()) {
    LOG(ERROR) << "PID State for new splice_info_section not found (pid = "
               << pid << ").";
    return;
  }

  LOG_IF(WARNING,splice_info->splice_command_type != 6) << "Unsupported splice command";
  LOG_IF(WARNING,splice_info->segmentation_descriptor_count > 1) << "Only supports single descriptor";
  LOG_IF(WARNING,splice_info->segmentation_descriptor[0].descriptor.splice_descriptor_tag != 0x02) << "Unsupported segmentation descriptor";

  // A callback from the a MediaHandler (demuxer) is needed to return the section data as an event

  // (ecl) Create a Scte35Event data object. For now the events are not queued since the break signals are not
  // continuously streamed so not adding them to a PidMap queue for now.

  std::shared_ptr<Scte35Event> event = std::make_shared<Scte35Event>();

  event->time_signal = splice_info->splice_command.splice_time_signal;
  event->descriptor = splice_info->segmentation_descriptor[0];
  event->start_time_pts = splice_info->pts_adjustment+((event->time_signal.time_specified_flag)?event->time_signal.time_spec.pts_time:0);
  // setting the event duration to the segmentation duration if present, otherwise using the pts_time, again if present, otherwise set to 0
  // TODO(ecl): verify this is correct. the segmentation duration may not equal the ad break duration if using a multi-segmented break sequence
  event->duration = (event->descriptor.segmentation_duration_flag)?event->descriptor.segmentation_duration:
    (event->time_signal.time_specified_flag)?event->time_signal.time_spec.pts_time:0; 
  

  new_signal_cb_.Run(event);

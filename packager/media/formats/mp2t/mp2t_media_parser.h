...
class Mp2tMediaParser : public MediaParser {
 public:
  ...
  /// Called when a new SCTE-35 section has been parsed.
  /// @param signal is the SCTE-35 event
  typedef base::Callback<void(const std::shared_ptr<Scte35Event>& signal)>
      NewSignalCB;
  ...
  void SetSignalCallback(const NewSignalCB& new_signal_cb);
 ...
 private:
  ...
  void OnNewSpliceInfoSection(uint32_t pid,

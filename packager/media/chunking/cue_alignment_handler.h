...
class CueAlignmentHandler : public MediaHandler {
 ...
 private:
  ...
  enum class CueState { kStateInProgram, kStateInAd };
  ...
  struct StreamState {
    ...
    // Cue state. While kStateInAd, each segment will be prefixed with a span tag with remaining duration. 
    // An end event will be ignored if not kStatInAd.
    CueState state = CueState::kStateInProgram;
  };
  ...
  Status OnSignal(std::unique_ptr<StreamData> data);
  ...
  void SetState(StreamState &stream, StreamDataType stream_type, 
      CueEventType cue_type, uint8_t seg_type) const;

  SyncPointQueue* sync_points_ = nullptr;

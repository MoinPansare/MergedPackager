...
const uint32_t kPtsTimescale = 90000;
...
struct Scte35Event {
  ...
  uint64_t duration;
  splice_time_t time_signal;
  segmentation_descriptor_t descriptor;

};

enum class CueEventType { kCueIn, kCueOut, kCuePoint, kCueScte35 };
...
struct CueEvent {
  ...
  double duration;
  std::string cue_data;
  // TODO(ecl): a shared pointer to the original scte35 event is included for SCTE-35 based cues. The SCTE-35 handler will create a Cue event to be used 
  // in the cue handling as a sync point event. This may be avoidable if a StreamData StreamDataType.scte35_event can be used instead
  std::shared_ptr<const Scte35Event> signal;
};
